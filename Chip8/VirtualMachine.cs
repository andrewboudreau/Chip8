...
namespace Chip8
{
    public class VirtualMachine
    {
        ...
        public VirtualMachine(State state, IScreen screen)
        {
            ...
            new Clock(Tick60Hz, EmulateOne, cancellationToken);
        }
        ...
        public void Execute(Instruction instruction)
        {
            switch (instruction.Code)
            {
                ...
                case 0xD:
                    ...
                    //Console.WriteLine($"{state.ProgramCounter:X4} - {instruction.Value:X4}\tDRW\tV{instruction.X:X1}({x}), V{instruction.Y:X1}({y}), {instruction.Nibs[3]}");
